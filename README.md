...
**① 法条检索 → 引用卡片**：跨 46 部受控语料（5,707 条条文条目）的 BM25 检索，每条结果带来源、时效与证据等级；摘要只显示程序统计，不调用生成模型。
...
应用内 [质量透明度页](/quality) 只读公示实时派生指标与带日期历史记录。自动化测试只能证明被覆盖的断言，不能证明系统「绝不出错」；检索评测（金标 263 组，hit@5 95.8%、rank-1 64.3%；2026-09-16 实测；指标随金标扩容与真实难例自然浮动）衡量的是语料命中率，不是法律正确率。
