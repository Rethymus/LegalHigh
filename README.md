...
**① 法条检索 → 引用卡片**：跨 108 部受控语料（10,669 条条文条目）的 BM25 检索，每条结果带来源、时效与证据等级；摘要只显示程序统计，不调用生成模型。
...
**知识导航特性**：法条详情页提供**双向交叉引用**（「本文引用→其他条文」与「其他条文→引用本文」，660 条已验证引用）；搜索框实时匹配 **215 张术语卡**引导从口语表述（如「定金能退吗」）到法条词面（「定金罚则」）；数据洞察页展示 **引用网络力导向图**（d3-force，31 部被引法律的共被引关系可视化）；法条与案例页支持**打印友好输出**（自动隐藏导航、保留证据链与引用来源）；flk native id 映射 **106/108 部**（证据账本 §19 source_native_id 全覆盖）。
...
| 浏览器巡检 / Route sweeps | 无头 Chrome 逐路由截图 + console/网络零错误门（58 路由，2026-09-24 实测 0 问题） |
| 对比度 / Contrast | WCAG AA 正文 4.5:1 + UI 指示器 3:1（strict 模式） |
| 动效探针 / Motion probes | 弹簧位移、Toast/Dialog 卸载、Reduce Motion 双通道等 11 项行为断言（2026-09-24 实测 11/11） |
| 可访问性探针 / A11y probes | WCAG 2.2 AA 子集（24px 目标尺寸 / 焦点不被遮挡；14 路由，2026-09-24 实测 0 违规） |
...
六工具：`search_articles`（BM25 全文检索法条原文）、`get_article`（按 ID+条号取原文与元数据）、`list_laws`（语料清单）、`search_cases`（指导案例与域外判例检索）、`search_history`（历史版本文本独立检索，非现行仅供对照）、`get_xrefs`（法条双向交叉引用导航，660 条已验证引用关系）。**只暴露检索，无任何生成型工具**；输出永远为法条原文+元数据+官方来源 URL，并声明「不构成法律意见」。详见 [v7 路线图](docs/plan/开发计划-v7-生态就绪路线图-2026-09.md) S2。
