...
```bash
# 测试与评测（103 项自动化测试，含检索金标 100 组 gate、审查点金标 58 组判定、案件分析红线自检、解读审核门、PIPL 通道）
...
| 模块 | 路由 | 说明 |
|---|---|---|
| 引用式问答 | `/qa` | BM25 检索 8 部法规 1,953 条，回答=命中条文原文卡片（法律名/条号/施行日期/来源链接）；**错误前提自动纠正 6 规则**（三十日退货/试用期一年/诉讼时效二年/扣押证件等，附真实条文）；检索不到明说，不生成 |
...
- 金标集：`server/tests/gold/gold_retrieval.json`，**100 组**「问题 → 应命中条文」（逐条与语料原文核对后标注，覆盖 8 部法律与三军民场景）+ 审查点判定表 58 组。
- 当前指标（实时计算，`/api/evals`）：**hit@5 = 0.98，MRR = 0.809**（100 组条文级口径；LegalBench-RAG 为字符级 span 口径，见 `docs/plan/开发计划.md` §四）。
...
server/
  ...
  tests/               # pytest（103 项）+ 金标集（检索 100 组/审查点 58 组判定）
web/
...
- **2026-08-30/31（查漏补缺 12 轮 + 决策执行）**：三透镜体检 38 项登记册、33 项落地（详见 `docs/plan/开发计划-v3-查漏补缺与打磨.md` 与 `docs/plan/决策清单-2026-08-30.md`）——检索统一 server BM25、解读库双轨+审核工作流、Word 修订双轨（导出+回传同步）、文书家族 7 模板、PIPL 删除/导出、工作身份档案、窄屏基线+大字模式、WCAG 全达标、一键五门 `run_qa.cmd`、安全扫描高危 0。
