...
REM Usage:  run_qa.cmd        run all 8 gates
REM         run_qa.cmd fast   skip visual sweep and behavior probes (run the first four gates)
...
echo [1/8] server pytest ...
cd server
.venv\Scripts\python.exe -m pytest tests -q
if errorlevel 1 set FAILED=1
cd ..

echo [2/8] web tsc + vite build ...
cd web
call npm run build
if errorlevel 1 set FAILED=1

echo [3/8] data-discipline grep gate ...
node scripts/qa_gates.mjs
if errorlevel 1 set FAILED=1

echo [4/8] WCAG contrast audit (tokens from global.css, --strict) ...
node scripts/qa_contrast.mjs --strict
if errorlevel 1 set FAILED=1

if /i "%1"=="fast" goto :summary
echo [5/8] read-only visual sweep (--strict; needs ports 8000/5173 up) ...
node scripts/qa_shots.mjs --strict
if errorlevel 1 set FAILED=1

echo [6/8] motion behavior probe (spring physics in headless Chrome; needs vite up) ...
node scripts/qa_motion.mjs --strict
if errorlevel 1 set FAILED=1

echo [7/8] a11y behavior probe (WCAG 2.2 AA subset: target size / focus not obscured; needs vite up) ...
node scripts/qa_a11y.mjs --strict
if errorlevel 1 set FAILED=1

echo [8/8] offline PWA probe (service worker + real offline navigation on vite preview; self-contained) ...
...
echo.
if /i "%1"=="fast" (
  echo FAST QA GATES PASSED ^(VISUAL SWEEP AND BEHAVIOR PROBES NOT RUN^)
) else (
