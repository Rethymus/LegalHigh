...
**① 法条检索 → 引用卡片**（全部视图可用）：跨 66 部受控语料（7,826 条条文条目）的 BM25 检索，每条结果带来源、时效与证据等级；摘要只显示程序统计，不调用生成模型。
...
- `web/public/data/laws-md/`：66 部法律全语料 markdown 导出（每法一文件，含官方元数据），与 `data/laws.json` 同源。
