...
[在线法条浏览](https://rethymus.github.io/LegalHigh/) · [v1.1.0-rc.1 预发布](https://github.com/Rethymus/LegalHigh/releases/tag/v1.1.0-rc.1) · [更新记录](CHANGELOG.md)

## 安装与版本选择

直接阅读法条可访问 GitHub Pages，站点仅提供静态浏览和产品说明。完整检索、事实梳理、合同审查、草稿、专业解读和可选 AI 需按下文步骤在本机运行 Python 后端。Release 提供源码与静态站 ZIP，并附 SHA-256；静态 ZIP 不是桌面安装包。v1.1.0-rc.1 是预发布，尚未完成三平台签名安装验收。旧 v1.0.0 不含后续修复。
...
以下截图采集于 2026-09-08，使用隔离本机后端。截图展示完整本机功能；GitHub Pages 仅提供其中的静态浏览部分。

| 首页（浅色） | 首页（深色） |
|---|---|
| ![首页·浅色](docs/qa-evidence/goal-2026-09-08-auth/01-dashboard.png) | ![首页·深色](docs/qa-evidence/goal-2026-09-08-auth/02-dashboard-dark.png) |

| 法律检索 | 法条详情 |
|---|---|
| ![法律检索](docs/qa-evidence/goal-2026-09-08-auth/06-search-results.png) | ![专业解读证据](docs/qa-evidence/goal-2026-09-08-expert/07b-law-detail-professional-evidence.png) |

| 合同审查 | 设置·外观（分段控件） |
|---|---|
| ![合同审查](docs/qa-evidence/goal-2026-09-08-auth/33-narrow-contract-review.png) | ![设置](docs/qa-evidence/goal-2026-09-08-auth/26-settings.png) |
...
以下资料用于方法、文档结构和评测设计参考；本仓库没有复制它们的数据或代码。2026-09-08 另核对 [CourtListener](https://github.com/freelawproject/courtlistener) 的项目结构、贡献与权利说明，以及 [docassemble](https://github.com/jhpyle/docassemble) 的引导式访谈定位和文档入口；本次采用清晰的使用方式入口、更新记录、升级说明与发布附件边界。证据等级：强（项目官方仓库），不表示对本项目背书。
