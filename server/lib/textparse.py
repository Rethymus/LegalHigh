...
_SOURCE_FURNITURE = (
    "本作品来自", "人民智云", "查论编", "责编：", "责编", "编辑：", "本作品是", "签署国务院令公布",
    # R291：「解读」移除——它是法定条文用语（法治宣传教育法 17/18/19 条正文中段出现即被
    # 截断，违反 R110「家具标记不得含法定条文用语」同款教训）；crpl-imp-2024 尾渣由
    # 「签署国务院令公布」标记继续覆盖（该尾渣块原文即含此串）。
...
def strip_source_furniture(text: str) -> str:
    ...
    for marker in _SOURCE_FURNITURE:
        ...
        if m:
            cut = min(cut, m.start())
    # R291：「解读」独立行 = crpl-imp-2024 尾渣形态（「施行。\n 解读\n 李强…」）——
    # 行中「解读」是法定用语（法治宣传教育法 17/18/19 条）不得触碰，故只按独立行锚定。
    m_jd = re.search(r"\n\s*解读\s*(\n|$)", text)
    if m_jd:
        cut = min(cut, m_jd.start())
