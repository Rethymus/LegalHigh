...
def main():
    ...
    # 上游转录缺陷的文本修正表（R280）——快照保持原始，修复住构建层（R139 先例）。
    # 键 = (law_id, 条号, 子条号)；值 = [(错, 正), …]（replace 全文应用，串需含足够上下文唯一化）。
    # 依据 = lawtext 第三链快照（flk DOCX 衍生社区转录，与 Wikisource 语料源上游独立）逐字
    # 比对 + 官方文本通例，全部为缺字/错字/标点级缺陷（证据：
    # docs/qa-evidence/lawtext-verify-cl-2026-09-21.json 与 docs/research/evidence/lawtext-cl-2020-*.md）。
    # cl-2023 第 165/166/169 条差异为刑修十二正当演进（语料=现行 2023 文本），不入此表；
    # 162之一/187 的 帐/账 为异体字（语料保持现代规范字），不入此表。
    TEXT_CORRECTIONS = {
        ("cl-2023", 36, ""): [("犯罪分子，同时被处罚金", "犯罪分子，同时被判处罚金")],
        ("cl-2023", 177, ""): [("变造金融票证的，处三年以下有期徒刑", "变造金融票证的，处五年以下有期徒刑")],
        ("cl-2023", 190, ""): [("责任人员处五年以下有期徒刑或者拘役", "责任人员，处五年以下有期徒刑或者拘役"),
                               ("责任人员处五年以上有期徒刑", "责任人员，处五年以上有期徒刑")],
        ("cl-2023", 198, ""): [("并处二万元以上十万元以下罚金或者没收财产", "并处二万元以上二十万元以下罚金或者没收财产")],
        ("cl-2023", 204, ""): [("定罪处罚，骗取税款超过所缴纳的税款部分", "定罪处罚；骗取税款超过所缴纳的税款部分")],
        ("cl-2023", 241, ""): [("收买被拐卖的妇女，儿童", "收买被拐卖的妇女、儿童")],
        ("cl-2023", 259, ""): [("奸淫现役军人妻子的", "奸淫现役军人的妻子的")],
        ("cl-2023", 261, ""): [("负有抚养义务而拒绝抚养", "负有扶养义务而拒绝扶养"),
                               ("处五年以下有期徒刑或者管制", "处五年以下有期徒刑、拘役或者管制")],
        ("cl-2023", 263, ""): [("方法抢劫财物的", "方法抢劫公私财物的")],
        ("cl-2023", 292, ""): [("第二百三十四条、条二百三十二条", "第二百三十四条、第二百三十二条")],
        ("cl-2023", 298, ""): [("造成公共秩序混乱的处五年以下", "造成公共秩序混乱的，处五年以下")],
        ("cl-2023", 308, "之一"): [("其他直接责任人员，依照第一款的规定处罚", "其他直接责任人员，依照第一款的规定处罚。")],
        ("cl-2023", 397, ""): [("处五年以下有期徒刑或者拘役；情节特别严重的，处三年以下有期徒刑或者拘役；情节特别严重的，处五年以上十年以下有期徒刑", "处五年以下有期徒刑或者拘役；情节特别严重的，处五年以上十年以下有期徒刑")],
        ("cl-2023", 323, ""): [("界埤、界桩", "界碑、界桩")],
        ("cl-2023", 331, ""): [("违反国务院卫生行政的有关规定", "违反国务院卫生行政部门的有关规定")],
        ("cl-2023", 344, "之一"): [("单处罚金。”", "单处罚金。")],
        ("cl-2023", 348, ""): [("处七年以上有期徒或者无期徒刑", "处七年以上有期徒刑或者无期徒刑")],
        ("cl-2023", 452, ""): [("有关刑事责任规定已纳入本法", "有关刑事责任的规定已纳入本法")],
        # scl-2012 两处句读（R287 定案）：lawtext（flk DOCX 衍生）与 lttxzmj 两条独立转录
        # 谱系一致为逗号，语料 Wikisource 源误作句号（2:1，证据 docs/qa-evidence/
        # lawtext-verify-3rd-scl-2012.json settlement_attempts/note）。
        ("scl-2012", 13, ""): [("作出是否赔偿的决定。赔偿义务机关", "作出是否赔偿的决定，赔偿义务机关")],
        ("scl-2012", 37, ""): [("提出支付申请。财政部门应当", "提出支付申请，财政部门应当")],
    }

    def apply_text_corrections(law: dict) -> int:
        n = 0
        for (lid, art_no, sub), ops in TEXT_CORRECTIONS.items():
            if lid != law["law_id"]:
                continue
            for a in law["articles"]:
                if a["no"] != art_no or (a.get("sub") or "") != sub:
                    continue
                for wrong, right in ops:
                    if wrong in a["text"]:
                        a["text"] = a["text"].replace(wrong, right)
                        n += 1
                    else:
                        raise ValueError(f"TEXT_CORRECTIONS 未命中：{lid}@{art_no}{sub} 串「{wrong[:20]}…」")
        return n

    manifest = []
    for law in laws:
        fixed = apply_text_corrections(law)
        if fixed:
            print(f"文本修正 {law['law_id']}: 应用 {fixed} 处（TEXT_CORRECTIONS，见 R280）")
