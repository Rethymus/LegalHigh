# -*- coding: utf-8 -*-
"""终验脚本：D8 删除链 / premise 纠错 / 评测指标 / A7 池 —— 全部走真实 API（证据输出）。
...
_ALLOWED_HOSTS = {"127.0.0.1", "localhost"}  # SSRF 防护：仅本机开发服务
...
def call(method: str, path: str, body: dict | None = None):
    if not path.startswith("/"):
        raise ValueError("path 必须以 / 开头（与 BASE 拼接，防路径外逃）")
...
c6 = "citation_pool" not in d and len(d["citation_laws"]) == 8
...
c8 = st == 200 and len(d["cases"]) >= 1 and all("保险" in (c["name"] + c["cause"] + c["summary"] + c["court"]) for c in d["cases"]) and all(c["verified"] for c in d["cases"])
