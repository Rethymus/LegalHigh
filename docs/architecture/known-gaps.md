...
9. **needs 取证词面误配 badcase（R165 judge 复核发现，登记不静默）**——极简描述「公司今天突然把我开除了」的取证条款出现《证券法》125/147（「被开除…不得招聘为从业人员」的「开除」词面撞车）。按硬规则⑲/金标先行制度**不在本轮静默改引擎**：处置=作为鸿沟实录数据资产登记，后续以「金标先行加题（目标=劳动合同法 39/47/87 等劳动法域条文）→ 评测 A/B → 凭数据开 ADR」推进；UI 侧已由「缺失关键事实清单」（R165，FLERF §8 unknown_material_facts）对冲——即便取证有噪声，系统也明确提示「片段事实不推定结论」。
