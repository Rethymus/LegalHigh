...
| # | 条件 | 证据指针 | 状态 |
|---|---|---|---|
| 1 | M6 遗留清零：flk 人工抽查 ≥10% 完成并归档；pcl-2023 末条定案并落地构建规则 | `docs/qa-evidence/flk-spotcheck-完成-2026-09-16.md` | ✅（2026-09-16：全语料级核验 26/28 部文本级双链一致 + flk 结构树 25/28 + 修十二 6/6，超额覆盖 ≥10% 抽样要求；pcl-2023 末条定案=保留附则说） |
...
## 五、转正三条件核对记录（S6-T1，定期执行）

### 2026-09-15 首次核对（rc.1 后第 7 天）

| # | 条件 | 当日状态 | 结论 |
|---|---|---|---|
| 1 | M6 遗留清零（flk 人工抽查 ≥10% + pcl-2023 末条定案） | ⬜ 未满足：167 条分层工作单已生成（2026-09-13），flk 抽查为业主侧人工动作，会话内不可代行；附则两说维持「待 flk 定案」口径 | 阻塞中（业主输入） |
| 2 | rc.1 起两周零回归 | 时间维度：rc.1=2026-09-08，核对日第 7 天，未满 14 天（2026-09-22 满）。质量维度：rc.1 起各轮（R36–R94）质量门持续全绿，无「因修复引入新缺陷」回归记录；R94 的 qa_motion 匹配器更新属探针随行为演进适配，非缺陷回归 | 质量维度持续满足，时间维度 2026-09-22 到期复核 |
| 3 | 云端 CI 连绿 | ✅ 近 30 条 Actions 运行 0 失败（qa + pages 双工作流，2026-09-09 起持续） | 持续满足 |

**当日终验**：`final_verify.py` 十项 ALL PASS（D8 PIPL 通道/premise 规则/金标 168 组 hit@5=0.9583/A7 引用池/解读双轨/C7 案例检索/场景拒绝公开），输出见会话运行记录。

**核对结论**：按决策 18「三条件齐即转正」——今日 **不转正**。条件 1 为唯一实质阻塞（业主侧 flk 抽查）；条件 2 于 2026-09-22 时间到期后复核即可。下一次核对时点：**2026-09-22**（条件 2 时间维度到期日）。
